...
1. Cookie (메모리 쿠키 - 파일 쿠키 (local 디스크에 txt 암호화 시켜서 저장 : 소멸타임)
	단점) 크기 제한적 (4KB), 보안 (사용자 삭제, 해커 파일 노출 위험)
2. local storage
	장점) 크기, 객체 사용 (데이터 : 의미있는 데이터 저장)
	
Server (웹 서버(WAS), : 메모리 + 파일(보조), DB서버)
1. server memory : session 객체 (사이트에 접속하는 사용자마다 고유하게 부여되는 메모리(객체))
   서버리부팅, 개발코드변경 >> 세션메모리 초기화
2. server memory : application 객체 (사이트에 접속하는 모든 사용자에게 부여되는 메모리(객체)
   서버리부팅, 개발코드변경 >> 세션메모리 초기화
3. server 영속적 저장 : file.txt (관리가 너무 힘듬)
...
<body>
	<%
		...
		response.addCookie(mycookie); // 한줄로 클라이언트에게 전달..!
	%>
